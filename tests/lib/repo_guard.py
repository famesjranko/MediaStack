...
SECRET_PATTERNS = [
    ...
    # then never `$` (shell interpolation — `${VAR:-...}` is this module's own
    # dominant shape for these keys) or `\` (a regex fragment like `\K`, e.g.
    # from a `grep -oP` extractor, not a value), then never this tree's two
    # hardcoded non-secret placeholders (env.example's "changeme" default and
    # dry-run.sh's "dry-run-key" stand-in), then 4+ non-whitespace,
    # non-quote, non-slash characters (excluding `/` keeps a `sed
    # 's/KEY=.*/KEY=value/'` line from swallowing the whole expression as
    # "the value").
    (
    ...
    (
        "api-key-assignment",
        r"(?<![A-Z0-9_])[A-Z][A-Z0-9_]*API_KEY\s*=\s*"
        r"['\"]?(?!\$)(?!\\)(?!dry-run-key\b)[^\s'\"/]{4,}",
    ),
    (
...
MEDIASTACK_EXAMPLE_PATH = r"(^|/)(docs|tests)/"
...
# only reads source-shaped files, and directory entries and non-regular files
# (symlinks, sockets) are skipped rather than opened.
def rule_secret_pattern(ctx: ScanContext) -> list[RuleFinding]:
    ...
    for path in ctx["worktree"]:
        ...
        if not os.path.isfile(os.path.join(ctx["root"], path)):
            continue
        is_example = example_path.search(path)
