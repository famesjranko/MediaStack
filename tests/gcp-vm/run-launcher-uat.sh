...
RC=$?
if (( RC == 0 )); then ok "Stage 1 install rc=0"; else bad "Stage 1 install rc=$RC (see $SETUP_LOG)"; exit "$RC"; fi
...
State left for the tester:
  ...
  ✗ Stage 2 NOT configured (REMOTE_WEB_STATE=unchecked)
      → testable via launcher: Add a feature → Add remote access
  ✗ Hardware transcoding NOT configured (no GPU on GCP VMs)
      → "Add hardware transcoding" should be HIDDEN from the submenu
...
Stage 2 credentials available in tests/.env.gcp:
  ...
  DDNS_USERNAME       Dynu API username
  DDNS_PASSWORD       Dynu API password
