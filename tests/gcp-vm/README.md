...
| 11 | Fail2ban: 4 jails                  | NPM/Jellyfin/Jellyseerr/ratelimit jails all active         |
...
> Jellyseerr 5055, Bazarr 6767, Radarr 7878, ddns-updater 8000,
...
the DNS provider. Default is **Dynu** (supports subdomains, free tier).

In your DDNS panel:
- Register your domain (e.g. `yourname.mywire.org`)
- Enable subdomains for `jellyfin.<domain>` and `jellyseerr.<domain>`
...
- **First-run vs re-run.** Many configurators detect existing state and
  skip (e.g. "Jellyseerr already initialized"). A fresh test always starts
