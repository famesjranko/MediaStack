# tests/scenarios/wireguard-server.sh — Server-tier firewallIps shape.
...
# possibly-500-but-persisted mutation path:
# wg-easy may answer 500 and still persist, so mutations are classified on read-back.
#
