# tests/scenarios/wireguard-containers.sh — Containers-tier firewallIps shape.
# Brings up wg-easy v15, enables the per-client firewall, creates a peer, and
# writes the containers-tier firewallIps (MediaStack container ports only —
# 51821 excluded so the peer can't manage other peers). Verifies the shape
# round-trips through wg-easy's possibly-500-but-persisted mutation path:
# wg-easy may answer 500 and still persist, so every mutation is classified on
# read-back, never on the HTTP status.
#
# Wall-time budget: ~3-5 min cold, ~1-2 min warm.

run_scenario() {
    # ------------------------------------------------------------------
    # 0. Prep: populate .env at the containers tier with firewall on.
    # ------------------------------------------------------------------
    ...
    env_set WG_ACCESS_TIER containers
    env_set WG_LAN_CIDR ""
    ...
    local family_fw_ips="127.0.0.1:80/tcp,127.0.0.1:81/tcp,127.0.0.1:443/tcp,127.0.0.1:3000/tcp,127.0.0.1:3001/tcp,127.0.0.1:5055/tcp,127.0.0.1:6767/tcp,127.0.0.1:7359/udp,127.0.0.1:7878/tcp,127.0.0.1:8000/tcp,127.0.0.1:8080/tcp,127.0.0.1:8090/tcp,127.0.0.1:8096/tcp,127.0.0.1:8191/tcp,127.0.0.1:8989/tcp,127.0.0.1:9000/tcp,127.0.0.1:9117/tcp"
...
print(json.dumps(d))')
    ...
    # ------------------------------------------------------------------
    # 3. Create a containers-tier peer (all MediaStack container ports on the
    # server IP). 51821 (wg-easy admin) is deliberately excluded so the peer
    # can't add or modify other peers.
    # ------------------------------------------------------------------
