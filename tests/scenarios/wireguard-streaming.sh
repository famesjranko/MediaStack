...
# peer, and writes the streaming-tier firewallIps (Jellyfin + Jellyseerr +
...
run_scenario() {
    # ------------------------------------------------------------------
    # 0. Prep: populate .env at the streaming tier with firewall on.
    # ------------------------------------------------------------------
    ...
    env_set WG_ACCESS_TIER streaming
    env_set WG_LAN_CIDR ""
    ...
    assert_eq "yes" "$admin_fw_state" "configure.sh initial admin peer firewallIps = Streaming tier"
...
print(json.dumps(d))')
    ...
    # ------------------------------------------------------------------
    # 3. Create a streaming-tier peer (Jellyfin + Jellyseerr + Homepage).
