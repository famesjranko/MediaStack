...
curl() {
    if [[ "$*" == *"__down"* ]]; then echo "12500000"; return 0; fi
    if [[ "$*" == *"__up"* ]]; then cat >/dev/null 2>&1; echo "5000000"; return 0; fi
    return 0
}
net_run_speedtest; rc=$?
...
curl() { cat >/dev/null 2>&1 || true; return 1; }   # primary method fails
timeout() { shift; "$@"; }                           # strip duration, resolve the stub fn
...
curl() {
    if [[ "$*" == *"__down"* ]]; then echo "not-a-number"; return 0; fi
    if [[ "$*" == *"__up"* ]]; then cat >/dev/null 2>&1; echo "5000000"; return 0; fi
}
librespeed-cli() { return 1; }
timeout() { shift; "$@"; }
...
# must be kept (#197 item 4/14). Under the old `|| return 1` the value was
# discarded and the whole method failed; librespeed-cli is absent here, so a
# regression surfaces as rc=1 with empty globals.
curl() {
    if [[ "$*" == *"__down"* ]]; then echo "1250000"; return 28; fi   # 10 Mbps, timed out
    if [[ "$*" == *"__up"* ]]; then cat >/dev/null 2>&1; echo "625000"; return 28; fi  # 5 Mbps
}
librespeed-cli() { return 1; }
timeout() { shift; "$@"; }
net_run_speedtest; rc=$?
