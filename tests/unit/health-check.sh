...
F2B_RUNNING=1; SVC_HEALTHY=1; SUDO_OK=1
...
RQ_FILE=$(mktemp); RQ_LAST=$(mktemp)
setq() { printf '%s\n' "$@" > "$RQ_FILE"; : > "$RQ_LAST"; }
_health_f2b_regex_count() {
    local n
    if [[ -s "$RQ_FILE" ]]; then n=$(head -1 "$RQ_FILE"); sed -i '1d' "$RQ_FILE"; printf '%s' "$n" > "$RQ_LAST"
...
UFW_INSTALLED=1; UFW_STATUS="Status: active"
CERT_BOUND="live/npm-1"; CERT_END="Jan  1 00:00:00 2099 GMT"; DU_JUMP=1; DU_RULES="-A X -p tcp -m multiport --dports 80 -j DROP"
sudo() {
    [[ "${1:-}" == "-n" ]] && shift
    case "${1:-}" in
        true)    return $((1 - SUDO_OK)) ;;
        sh)      [[ "$*" == *ufw* ]] && return $((1 - UFW_INSTALLED)); return 0 ;;
        ufw)     printf '%s\n' "$UFW_STATUS" ;;
        grep)    printf '%s\n' "$CERT_BOUND" ;;
...
_v() { local exp="$1" label="$2"; shift 2; local out st; out=$("$@"); st="${out%%|*}"; assert_eq "$exp" "$st" "$label -> $out"; }

# --- fail2ban regex (delta) -----------------------------------------------
setq 0 1; _v ok   "regex: fresh line increments count => ok" health_fail2ban_regex jellyfin
setq 3 3; _v fail "regex: count flat despite history => fail (no false-pass)" health_fail2ban_regex jellyfin
F2B_RUNNING=0;    _v skip "regex: fail2ban down => skip" health_fail2ban_regex jellyfin; F2B_RUNNING=1
SVC_HEALTHY=0;    _v skip "regex: service not ready => skip" health_fail2ban_regex jellyfin; SVC_HEALTHY=1
...
# --- dns drift -------------------------------------------------------------
DOMAIN="example.org"
DNS_TOKEN="ok";            _v ok   "dns: match => ok" health_dns_drift
DNS_TOKEN="mismatch:1.2.3.4"; _v fail "dns: mismatch => fail" health_dns_drift
DNS_TOKEN="no-a";          _v warn "dns: missing A-records => warn" health_dns_drift
DOMAIN="";                 _v skip "dns: no domain => skip" health_dns_drift; DOMAIN="example.org"

# --- disk % ----------------------------------------------------------------
DF_PCT=40; _v ok   "disk: 40% => ok"   health_disk_pct
DF_PCT=88; _v warn "disk: 88% => warn" health_disk_pct
DF_PCT=97; _v fail "disk: 97% => fail" health_disk_pct

# --- cert expiry -----------------------------------------------------------
CERT_END="Jan  1 00:00:00 2099 GMT"; _v ok   "cert: far future => ok" health_cert_expiry
CERT_END="$(date -d '+5 days' '+%b %e %H:%M:%S %Y GMT')"; _v warn "cert: <14d => warn" health_cert_expiry
CERT_END="$(date -d '-2 days' '+%b %e %H:%M:%S %Y GMT')"; _v fail "cert: expired => fail" health_cert_expiry
CERT_BOUND=""; _v skip "cert: no proxy hosts => skip" health_cert_expiry; CERT_BOUND="live/npm-1"
SUDO_OK=0; _v skip "cert: no sudo => skip" health_cert_expiry; SUDO_OK=1
DOMAIN=""; _v skip "cert: no domain => skip" health_cert_expiry; DOMAIN="example.org"

# --- ufw -------------------------------------------------------------------
UFW_STATUS="Status: active";   _v ok   "ufw: active => ok"   health_ufw_active
UFW_STATUS="Status: inactive"; _v fail "ufw: inactive => fail" health_ufw_active
UFW_INSTALLED=0; _v skip "ufw: not installed => skip" health_ufw_active; UFW_INSTALLED=1
SUDO_OK=0; _v skip "ufw: no sudo => skip" health_ufw_active; SUDO_OK=1

# --- docker-user restrict --------------------------------------------------
_v ok "docker-user: jump + DROP rules => ok" health_docker_user_restrict
DU_JUMP=0; _v fail "docker-user: jump missing => fail" health_docker_user_restrict; DU_JUMP=1
DU_RULES="-A X -j RETURN"; _v fail "docker-user: chain empty (Docker flushed) => fail" health_docker_user_restrict
DU_RULES="-A X -p tcp -m multiport --dports 80 -j DROP"
SUDO_OK=0; _v skip "docker-user: no sudo => skip" health_docker_user_restrict; SUDO_OK=1

# --- health_present maps STATUS -> log_* -----------------------------------
LP=""; log_ok(){ LP="ok:$*"; }; log_warn(){ LP="warn:$*"; }; log_error(){ LP="err:$*"; }; log_skip(){ LP="skip:$*"; }
health_present "ok|good";   assert_eq "ok:good"   "$LP" "present: ok => log_ok"
health_present "fail|bad";  assert_eq "err:bad"   "$LP" "present: fail => log_error"
health_present "skip|n/a";  assert_eq "skip:n/a"  "$LP" "present: skip => log_skip"
