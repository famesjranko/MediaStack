...
assert_contains "$stage2_source" "_stage2_install()" "install function exists"
assert_contains "$stage2_source" 'MEDIASTACK_NPM_ATTEMPT_REMOTE=$attempt_remote ./scripts/configure.sh --only npm,ddns-updater,wireguard' "NPM remote attempt is process-scoped (Dynu=1; unverified non-Dynu=0)"
assert_contains "$stage2_source" "type ui_spin" "Stage 2 remote attempt falls back when UI spinner is not loaded"
assert_contains "$stage2_source" '_stage2_le_ready_hosts' "ready promotion checks NPM disk/proxy postconditions"
assert_contains "$stage2_source" '_stage2_probe_https_ready "https://$fqdn"' "ready promotion checks live HTTPS"
assert_contains "$stage2_source" "_stage2_set_remote_state ready" "install can promote ready after postconditions"
assert_contains "$stage2_source" "_stage2_set_remote_state failed" "install records failed state after cert failure"
assert_contains "$stage2_source" "stage2_le_gate" "install delegates certificate postconditions to LE gate"
assert_contains "$stage2_source" "config/state/npm-cert-status-last.json" "LE gate exposes persistent cert status path"
assert_contains "$stage2_source" "stage2_le_failure_copy" "LE classifications have user-facing copy helper"
