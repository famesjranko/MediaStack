...
if [[ -n "$_utf8_locale" ]]; then
    assert_eq "3" "$(
        ...
        _ui_visible_len "$(printf '\033[1mABC\033[0m')"
    )" \
...
widths="$(printf '%s\n' "$box" | grep -E '│|╭|╰' | while IFS= read -r l; do
    _oracle_len "$l"
    echo
done | sort -u)"
distinct="$(printf '%s\n' "$widths" | grep -c .)"
assert_eq "1" "$distinct" "ui_box+ui_kv: all box lines render at one visible width"

# --- The two longest summary keys align their value column with shorter rows ---
...
assert_eq "$(_ui_kv_value_col 'Homepage')" "$(_ui_kv_value_col 'Hardware transcoding')" \
    "ui_kv: 'Hardware transcoding' (20c) value column aligns with shorter rows"
assert_eq "$(_ui_kv_value_col 'Domain')" "$(_ui_kv_value_col 'Remote streaming cap')" \
    "ui_kv: 'Remote streaming cap' (20c) value column aligns with shorter rows"
