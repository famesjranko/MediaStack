...
- inotify-tools (inotifywait — the fail2ban log-rotation reload watcher, `mediastack-fail2ban-reload.service`, watches for new date-stamped log files and reloads fail2ban so jail globs re-resolve; a `mediastack-fail2ban-reload-fallback.timer` also reloads every 6h as a floor if the watcher ever dies; issue #291)
