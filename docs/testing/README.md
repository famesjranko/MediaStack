...
- `dind_build` — build `ms-dind:debian` from `tests/Dockerfile.dind` on every `run.sh` invocation; docker's layer cache makes an unchanged Dockerfile a fast no-op, while a changed `Dockerfile.dind`/`dind-entrypoint.sh` rebuilds (a failed rebuild aborts the run rather than silently reusing the stale image). All runtime deps (`bash`, `python3`, `python3-yaml`, `curl`, `grep`, `sed`, `gettext-base`, `docker-ce` + `docker-compose-plugin`, `iptables`, `pigz`, `xz-utils`) are pre-baked into the image.
