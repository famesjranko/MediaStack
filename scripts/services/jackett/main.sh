...
configure_jackett() {
    ...
    local jackett_url="http://localhost:9117/api/v2.0/indexers"
    ...
    curl -sf -c "$jar" -X POST "http://localhost:9117/UI/Dashboard" -d "" >/dev/null 2>&1
    ...
    if [[ "$probe_code" != "200" && -n "$jf_pw" ]]; then
        curl -sf -c "$jar" -X POST "http://localhost:9117/UI/Dashboard" \
...
" 2>/dev/null || echo "")
    ...
    if [[ -n "$current_flare" ]]; then
        ...
        server_cfg=$(curl -sf -b "$jar" \
            "http://localhost:9117/api/v2.0/server/config?apikey=${jackett_key}" 2>/dev/null)
        if [[ -n "$server_cfg" ]]; then
            local patched
            patched=$(echo "$server_cfg" | python3 -c "
import sys, json
...
" 2>/dev/null)
            if [[ -n "$patched" ]] && curl -sf -b "$jar" -X POST \
                "http://localhost:9117/api/v2.0/server/config?apikey=${jackett_key}" \
                -H "Content-Type: application/json" \
                -d "$patched" >/dev/null 2>&1; then
                log_ok "FlareSolverr connected: http://flaresolverr:8191"
            else
...
" 2>/dev/null || echo "")
        ...
        if [[ -n "$current_hash" ]]; then
            ...
            if curl -sf -b "$jar" -X POST \
                "http://localhost:9117/api/v2.0/server/adminpassword?apikey=${jackett_key}" \
                -H "Content-Type: application/json" \
