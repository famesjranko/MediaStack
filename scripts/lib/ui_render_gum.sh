...
_render_spin_demo() {
    ...
    local secs
    secs=$(printf '%.0f' "${delay:-0}")
    ((secs < 1)) && secs=1
    gum spin --spinner dot --title "  $title" -- sleep "$secs" 2>/dev/null || true
}

_render_spin() {
    # Deliberately NOT `gum spin`: gum's spin has Ctrl-C bugs
    # (https://github.com/charmbracelet/gum/issues/690 — doesn't kill a child whose
    # stdout is redirected; https://github.com/charmbracelet/gum/issues/588 —
    # non-130 exit on Ctrl-C) and can't wrap shell functions. Use our own bash spinner (same as
...
_render_password() {
    local prompt="$1" default="${2:-}" result rc
    result=$(gum input --password --placeholder "••••••" --prompt "  $prompt: ")
    rc=$?
    _gum_reraise_on_sigint "$rc"
    ((rc == 130)) && return 130 # Ctrl-C: distinct abort (see _render_choose)
    ((rc != 0)) && return 3
...
_render_choose() {
    ...
    local result rc
    result=$(gum choose --header "  $prompt" --height="$height" "${selected_args[@]}" "${items[@]}")
    rc=$?
    _gum_reraise_on_sigint "$rc"
